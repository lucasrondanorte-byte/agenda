    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 z-50 flex justify-center items-center p-4" onClick={onClose}>
      <div className="bg-white rounded-lg shadow-xl p-6 w-full max-w-lg" onClick={e => e.stopPropagation()}>
        <div className="flex justify-between items-center mb-6">
            <h2 className="text-2xl font-bold text-slate-800">{projectToEdit ? 'Editar Proyecto' : 'Nuevo Proyecto'}</h2>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div>
            <label className="block text-sm font-medium text-slate-700 mb-2">Icono del Proyecto</label>
            <div className="flex flex-wrap gap-2 bg-slate-50 p-3 rounded-lg">
                {projectIcons.map(i => (
                    <button 
                        ...
                        type="button" 
                        onClick={() => setIcon(i)}
                        className={`w-10 h-10 text-xl rounded-full transition-all ${icon === i ? 'bg-indigo-200 scale-110' : 'hover:bg-slate-200'}`}
                    >{i}</button>
                ))}
            </div>
          </div>
          <div>
            <label htmlFor="project-title" className="block text-sm font-medium text-slate-700 mb-1">Título</label>
           ...
           <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
             <div>
                <label htmlFor="project-category" className="block text-sm font-medium text-slate-700 mb-1">Categoría (Opcional)</label>
             ...
             <div>
                <label htmlFor="project-date" className="block text-sm font-medium text-slate-700 mb-1">Fecha Límite (Opcional)</label>
                <input type="date" id="project-date" value={targetDate} onChange={(e) => setTargetDate(e.target.value)}
                  className="w-full px-3 py-2 border border-slate-300 rounded-md shadow-sm focus:outline-none focus:ring-2 focus:ring-indigo-500"/>
             </div>
           </div>
          <div>
            <label htmlFor="project-description" className="block text-sm font-medium text-slate-700 mb-1">Descripción (Opcional)</label>
