    ...
    return (
        <div className="flex items-center group w-full py-1">
            ...
            <div className="ml-3 flex-grow">
                {isEditing ? (
                    <input type="text" value={editText} onChange={e => setEditText(e.target.value)} onBlur={handleSave} onKeyDown={e => e.key === 'Enter' && handleSave()} autoFocus className="w-full bg-transparent p-1 -m-1 border-b border-indigo-500 outline-none text-sm"/>
                ) : (
                    <p onDoubleClick={() => setIsEditing(true)} className={`text-sm cursor-pointer ${item.completed ? 'line-through text-slate-400' : 'text-slate-700'}`}>{item.text}</p>
                )}
            </div>
            <div className="flex items-center space-x-1 opacity-0 group-hover:opacity-100 transition-opacity ml-2">
                ...
                <button onClick={() => onDelete(listId, item.id)} className="p-1 text-slate-400 hover:text-red-600 rounded"><TrashIcon className="w-4 h-4"/></button>
            </div>
        </div>
    );
};
        ...
        <div className="bg-white rounded-xl shadow-sm p-4 flex flex-col">
            <div className="flex justify-between items-center mb-3">
                ...
                <div className="flex items-center">
                     <button onClick={onEditList} className="p-1.5 text-slate-400 hover:text-indigo-600 rounded-md"><PencilIcon className="w-4 h-4"/></button>
                     <button onClick={onDelete} className="p-1.5 text-slate-400 hover:text-red-600 rounded-md"><TrashIcon className="w-4 h-4"/></button>
                </div>
            </div>
            <div className="flex-grow space-y-1 pr-1 overflow-y-auto max-h-60">
                ...
                )) : <p className="text-center text-xs text-slate-400 py-4">¡Todo listo!</p>}
            </div>
            {completedItems.length > 0 && (
                <div className="mt-3 pt-3 border-t border-slate-200">
                    <button onClick={() => setShowCompleted(!showCompleted)} className="w-full flex justify-between items-center text-sm font-medium text-slate-500">
            ...
            <form onSubmit={handleAddItem} className="mt-3 pt-3 border-t border-slate-200 flex items-center space-x-2">
                <input type="text" value={newItemText} onChange={e => setNewItemText(e.target.value)} placeholder="Añadir elemento..." className="w-full px-3 py-1.5 bg-slate-50 border border-slate-200 rounded-md shadow-sm text-sm focus:outline-none focus:ring-1 focus:ring-indigo-500" />
                <button type="submit" className="p-2 bg-indigo-600 text-white rounded-md hover:bg-indigo-700"><PlusIcon className="w-4 h-4"/></button>
            </form>
        </div>
    );
};
...
export const HomePanel: React.FC<HomePanelProps> = (props) => {
    ...
    return (
        <div className="bg-slate-50/50 p-6 rounded-xl shadow-inner">
            <div className="flex justify-between items-center mb-6">
                <h3 className="text-2xl font-bold text-slate-800">Mi Hogar: Listas y Tareas</h3>
                <button onClick={() => handleOpenForm()} className="flex items-center space-x-2 px-4 py-2 bg-indigo-600 text-white rounded-md text-sm font-medium shadow-sm hover:bg-indigo-700">
            ...
            {showForm && (
                <form onSubmit={handleSave} className="mb-6 p-4 bg-white rounded-lg shadow-md border border-slate-200 space-y-4">
                    <h4 className="font-semibold">{editingList ? 'Editar Lista' : 'Nueva Lista'}</h4>
                     <div>
                        ...
                        <div className="flex flex-wrap gap-2">
                            {listIcons.map(i => (
                                <button key={i} type="button" onClick={() => setIcon(i)} className={`w-10 h-10 text-xl rounded-full transition-all ${icon === i ? 'bg-indigo-200 scale-110' : 'hover:bg-slate-200'}`}>{i}</button>
                            ))}
                        </div>
                     </div>
                     <div>
                        <label htmlFor="list-title" className="block text-sm font-medium text-slate-700 mb-1">Título</label>
                        <input id="list-title" type="text" value={title} onChange={e => setTitle(e.target.value)} placeholder="p. ej. Supermercado" className="w-full px-3 py-2 border border-slate-300 rounded-md shadow-sm focus:outline-none focus:ring-2 focus:ring-indigo-500" />
                     </div>
                     <div className="flex justify-end space-x-2">
                        <button type="button" onClick={handleCloseForm} className="px-3 py-1.5 bg-white border border-slate-300 rounded-md text-sm font-medium text-slate-700 hover:bg-slate-50">Cancelar</button>
                        <button type="submit" className="px-3 py-1.5 bg-indigo-600 text-white rounded-md text-sm font-medium shadow-sm hover:bg-indigo-700">Guardar</button>
                     </div>
                </form>
            )}
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                 ...
                 <div className="text-center py-16 border-2 border-dashed border-slate-300 rounded-lg">
                    <div className="mx-auto w-12 h-12 bg-slate-100 rounded-full flex items-center justify-center">
                         <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth="1.5" stroke="currentColor" className="w-6 h-6 text-slate-400"><path strokeLinecap="round" strokeLinejoin="round" d="M8.25 6.75h7.5M8.25 12h7.5m-7.5 5.25h7.5M3.75 6.75h.007v.008H3.75V6.75Zm.375 0a.375.375 0 1 1-.75 0 .375.375 0 0 1 .75 0ZM3.75 12h.007v.008H3.75V12Zm.375 0a.375.375 0 1 1-.75 0 .375.375 0 0 1 .75 0Zm-.375 5.25h.007v.008H3.75v-.008Zm.375 0a.375.375 0 1 1-.75 0 .375.375 0 0 1 .75 0Z" /></svg>
                    </div>
                    <h4 className="mt-4 text-lg font-semibold text-slate-700">Empieza a organizarte</h4>
                    <p className="mt-1 text-sm text-slate-500">Crea tu primera lista para llevar un registro de todo.</p>
                </div>
            )}
        </div>
    );
